...
(async () => {
  for (const f of files) {
    ...
    const isTread = (i) => src[i] < 80 && (src[i]-src[i+2]) < 16 && src[i] > g[0] + 6 && src[i] < 200; // dim near-neutral: the drawn treadmill
    const trav = (i) => !isGround(i) && !isDark(i) && !isTread(i);
    ...
    for (let y = y0; y <= y1; y++) for (let x = 0; x < W; x++) {
      const p = y*W+x, i = p*C; if (isGround(i)) continue; if (seen[p] && !isTread(i)) continue;
      data[i]=g[0]; data[i+1]=g[1]; data[i+2]=g[2];
    }
